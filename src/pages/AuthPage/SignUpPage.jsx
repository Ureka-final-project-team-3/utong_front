...
const SignupPage = () => {
  ...
  const handleChange = (field, value) => {
    ...
    switch (field) {
      case 'email':
        if (!value) newErrors.email = '아이디를 입력해주세요.';
      ...
      case 'password':
        ...
        else if (value.length < 8) newErrors.password = '비밀번호는 8자 이상이어야 합니다.';
      ...
      case 'passwordConfirm':
        if (value !== password) newErrors.passwordConfirm = '비밀번호가 일치하지 않습니다.';
        else delete newErrors.passwordConfirm;
        break;
      case 'nickname':
        if (!value) newErrors.nickname = '닉네임을 입력해주세요.';
        else delete newErrors.nickname;
        break;
      case 'phone':
        if (!value) newErrors.phone = '전화번호를 입력해주세요.';
  ...
  const handleSignup = async () => {
    ...
    else if (!/\S+@\S+\.\S+/.test(email)) newErrors.email = '유효한 이메일 형식을 입력해주세요.';
